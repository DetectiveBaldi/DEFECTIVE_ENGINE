...
3. Build the game by running the following command in a command prompt (replace :target: with a specific target like html5, cpp, etc.): `haxelib run lime build :target:`
...
## Notice
This engine uses songs from various mods unaffiliated with Defective Engine for testing purposes. These songs will be removed when the engine is closer to a finished state.